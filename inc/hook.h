...
// 创建与原来函数签名 一样的函数指针，并预先声明一个函数用于保存原来的函数调用
// sleep
...
// socket:
typedef int (*socket_fun)(int somain, int type, int protocal);
...
typedef ssize_t (*readv_fun)(int fd, const struct iovec* iov, int iocnt);
...
typedef ssize_t (*writev_fun)(int fd, const struct iovec* iov, int iocnt);
extern writev_fun writev_f;

typedef ssize_t (*send_fun)(int s, const void* msg, size_t len, int flags);
extern send_fun send_f;

typedef ssize_t (*sendto_fun)(int s, const void* msg, size_t len, int flags, const sockaddr* to, socklen_t tolen);
